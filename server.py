...
async def translate(text, src_lang, src_name, tgt_lang, tgt_name):
    """Google önce, hata varsa Anthropic"""
    # Önce Google Translate dene
    for attempt in range(3):
        try:
            result = await google_translate(text, src_lang, tgt_lang)
            if result:
                print(f"Google çeviri OK: {src_name}→{tgt_name}", flush=True)
                return result
        except Exception as e:
            print(f"Google hata (deneme {attempt+1}): {e}", flush=True)
            if attempt < 2:
                await asyncio.sleep(1)

    # Google başarısız → Anthropic'i dene
    print("Anthropic'e geçiliyor...", flush=True)
    try:
        result = await anthropic_translate(text, src_name, tgt_name)
        print("Anthropic çeviri OK", flush=True)
        return result
    except Exception as e:
        print(f"Anthropic da hata: {e}", flush=True)
        raise Exception(f"Çeviri başarısız: {e}")
