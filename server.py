...
async def translate(text, src_name, tgt_name):
    print(f"translate() cagriliyor: {src_name}→{tgt_name}, key={'OK' if ANTHROPIC_KEY else 'EKSIK'}", flush=True)
    payload = {
    ...
    print(f"translate() basarili", flush=True)
    ...
    try:
        async for msg in ws:
            ...
            if action == "create":
                ...
                print(f"[{my_room}] CEVIRILIYOR: {src_name}→{tgt_name}: {text[:40]}", flush=True)
                ...
                if src_name == tgt_name:
                    ...
                    try:
                        ...
                        import traceback
                        print(f"[{my_room}] CEVIRI HATASI: {type(e).__name__}: {e}", flush=True)
                        traceback.print_exc()
                        await safe_send(ws, {"action": "error", "msg": f"Çeviri hatası: {type(e).__name__}: {e}"})
                        continue
